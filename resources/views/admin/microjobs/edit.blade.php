...
@section('title', 'Edit Microjob')
@section('page_title', 'Update Microtask')
        ...
        <div class="col-md-8">
            <div class="card-modern">
                <div class="card-header bg-white border-bottom py-3 d-flex justify-content-between align-items-center">
                    ...
                    <span class="badge bg-{{ $job->status === 'approved' ? 'success' : ($job->status === 'pending' ? 'warning' : 'danger') }}-soft text-{{ $job->status === 'approved' ? 'success' : ($job->status === 'pending' ? 'warning' : 'danger') }} rounded-pill px-3">
                        {{ ucfirst($job->status) }}
                    </span>
                </div>
                <div class="card-body p-4">
                    <form action="{{ route('admin.microjobs.update-job', $job->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PATCH')
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="title" class="form-control rounded-pill px-4 @error('title') is-invalid @enderror" 
                                   placeholder="e.g. Subscribe to YouTube Channel" value="{{ old('title', $job->title) }}" required>
                            @error('title')
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="description" class="form-control rounded-4 px-4 py-3 @error('description') is-invalid @enderror" 
                                      rows="5" placeholder="Step by step instructions for the worker..." required>{{ old('description', $job->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback ml-3">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="url" name="job_url" class="form-control rounded-pill px-4 @error('job_url') is-invalid @enderror" 
                                   placeholder="https://example.com/job-link" value="{{ old('job_url', $job->job_url) }}" required>
                            @error('job_url')
                                <div class="invalid-feedback ml-3">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="row mb-4">
                            <div class="col-md-6">
                                ...
                                <select name="is_active" class="form-select rounded-pill px-4">
