...
@section('page_title', 'Deposit Approvals')
...
<div class="fade-in">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            ...
            <p class="text-muted small mb-0">Review and approve manual add-money deposits</p>
        </div>
        <div class="d-flex gap-2">
            <a href="{{ route('admin.money-requests.index', ['status' => 'Pending']) }}"
               class="btn btn-{{ $status === 'Pending' ? 'warning' : 'light' }} rounded-pill px-4 shadow-sm">
                <i class="fa-solid fa-clock me-2"></i>Pending
            </a>
            <a href="{{ route('admin.money-requests.index', ['status' => 'Approved']) }}"
               class="btn btn-{{ $status === 'Approved' ? 'success' : 'light' }} rounded-pill px-4 shadow-sm">
                <i class="fa-solid fa-check me-2"></i>Approved
            </a>
            <a href="{{ route('admin.money-requests.index', ['status' => 'Rejected']) }}"
                    ...
                    <tr>
                        ...
                        <td class="px-4 py-3 text-end">
                            ...
                            <div class="d-flex justify-content-end gap-2">
                                <form action="{{ route('admin.money-requests.update', $req->id) }}" method="POST">
                                    @csrf
                                    @method('PATCH')
                                    <input type="hidden" name="action" value="Approved">
                                    <button type="submit" class="btn btn-success btn-sm rounded-pill px-3 shadow-sm" onclick="return confirm('Approve this ৳{{ $req->amount }} deposit?')">
                                ...
                                <form action="{{ route('admin.money-requests.update', $req->id) }}" method="POST">
                                    ...
                                    <button type="submit" class="btn btn-danger btn-sm rounded-pill px-3 shadow-sm" onclick="return confirm('Reject this request?')">
